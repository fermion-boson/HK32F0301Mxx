...
/* SPI configuration ------------------------------------------------------*/
  ...
  while (1)
  {
    ...
    {    
      ...
      {
        /* 通过查询TXE标志，发送【DATA_SIZE】+ 1数量的数据 */
        if((Tx_Idx < DATA_SIZE) && (SPI_GetFlagStatus(SPI_SEL, SPI_FLAG_TXE) == SET))
        {
          if (CmdStatus == 0x00)
          {
        ...
        /* 通过查询RXNE标志，接收【DATA_SIZE】+ 1数量的数据 */
        if(SPI_GetFlagStatus(SPI_SEL, SPI_FLAG_RXNE) == SET)
        {
          if (CmdReceived == 0x00)
          {
...
//    while(1);
  ...
  {     
    ...
    by the Master */
    /* 通过查询TXE标志，发送1byte的数据 */
    if (SPI_GetFlagStatus(SPI_SEL, SPI_FLAG_TXE) == SET)
    {
...
//          printf("Slave receive data : 0x%02x \r\n",CmdReceived);
    ...
    while ((Rx_Idx < DATA_SIZE)&&(TimeOut != 0x00))
    {
      /* 通过查询TXE标志，发送【DATA_SIZE】数量的数据 */
      if((Tx_Idx < DATA_SIZE) && (SPI_GetFlagStatus(SPI_SEL, SPI_FLAG_TXE) == SET))
      {
        SPI_SendData8(SPI_SEL, TxBuffer[Tx_Idx++]);
      }
      
      /* 通过查询RXNE标志，接收【DATA_SIZE】数量的数据 */
      if (SPI_GetFlagStatus(SPI_SEL, SPI_FLAG_RXNE) == SET)
      {
        RxBuffer[Rx_Idx++] = SPI_ReceiveData8(SPI_SEL);
//        printf("Slave receive data : 0x%02x \r\n",RxBuffer[Rx_Idx]);
...
static void SPI_Config(void)
{
  ...
  RCC_APBPeriph2ClockCmd(RCC_APBPeriph2_SPI, ENABLE);
