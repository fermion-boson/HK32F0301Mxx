...
//	FLASH_Lock();
	...
	if(1 == EEPROM_ReadByte(0,&eeprom_init_flag))
	{
		if(EEPROM_INIT_FLAG == eeprom_init_flag)
		{
			...
			for(i=10;i<30;i++)
			{
				if(1 == EEPROM_ReadByte(HK32F0301M_EE_SIZE-i,&eeprom_init_flag))
				{
					printf("offset_address: %d, data: %d\r\n",HK32F0301M_EE_SIZE-i,eeprom_init_flag);
				}
			}
		}
		else
		{
			...
			if( 1== EEPROM_WriteByte(0,EEPROM_INIT_FLAG))
			{
				...
				if(i == 10)
				{
					for(i=10;i<30;i++)
					{
						if(0 == EEPROM_WriteByte(HK32F0301M_EE_SIZE-i,i))
						{
							printf("EEPROM data write fail!\r\n");
						}
					}
