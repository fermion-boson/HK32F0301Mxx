...
//  ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T1_TRGO;
  ...
  ADC_ChannelConfig(ADC, ADC_Channel_2 , ADC_SampleTime_239_5Cycles);//配置采样速率和通道

	ADC_AnalogWatchdogThresholdsConfig(ADC, 3102, 1861);//输入ADCwatchdog高低阀值
	ADC_AnalogWatchdogSingleChannelCmd(ADC, ENABLE);//开启ADCwatchdog
	ADC_AnalogWatchdogSingleChannelConfig(ADC, ADC_AnalogWatchdog_Channel_2);//选择ADCwatchdog_Channel2
	
	/* Enable AWD interrupt */
  ADC_ITConfig(ADC, ADC_IT_AWD, ENABLE);//使能中断
