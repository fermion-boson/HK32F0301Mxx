...
{
	while((UART2->ISR & UART_ISR_TXE) == 0);
	UART1->TDR = ch;
}
...
static void UART_GPIO_Configurature(void)
{
	...
	m_gpio.GPIO_Pin = UART2_TX_PIN;
	m_gpio.GPIO_PuPd = GPIO_PuPd_NOPULL;
	m_gpio.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(UART2_TX_PORT, &m_gpio);
	GPIO_PinAFConfig(UART2_TX_PORT,GPIO_PinSource3,GPIO_AF_6);
	
	m_gpio.GPIO_Pin = UART2_RX_PIN;
	GPIO_Init(UART2_RX_PORT, &m_gpio);
	GPIO_PinAFConfig(UART2_RX_PORT,GPIO_PinSource6,GPIO_AF_6);
}
...
{
	uint8_t ch_rx_test;
	if((UART2->ISR & UART_ISR_RXNE) != 0)
	{
		ch_rx_test = UART2->RDR;
...
{
    /*  Printf   */
    UART_SendData(UART2, (unsigned char) ch);
    while(UART_GetFlagStatus(UART2, UART_FLAG_TC) != SET);
    return (ch);
}
/*******************************************************************************************************
* @function: fgetc
...
int fgetc(FILE *f)
{
	while((UART2->ISR & UART_ISR_RXNE) == 0); 
	return (int)((uint8_t)(UART2->RDR));
}
