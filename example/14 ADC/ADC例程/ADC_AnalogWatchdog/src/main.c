...
void ADC_Configuration(void)
{
  ...
  ADC_ChannelConfig(ADC1, ADC_Channel_2 , ADC_SampleTime_239_5Cycles);//配置采样速率和通道

	ADC_AnalogWatchdogThresholdsConfig(ADC1, 3102, 1861);//输入ADCwatchdog高低阀值
  ...
  ADC_ITConfig(ADC1, ADC_IT_AWD, ENABLE);//使能中断
