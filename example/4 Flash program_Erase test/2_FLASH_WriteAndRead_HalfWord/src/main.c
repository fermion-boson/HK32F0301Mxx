...
void Write_Flash(uint32_t FlashAddress,uint8_t *WriteBuff,uint16_t Size)
{
	...
	{
		FLASH_ProgramHalfWord(FlashAddress + j, WriteBuff[j] + (WriteBuff[j+1]<<8));
	}
...
void Flash_WriteData(void)
{
	...
	printf(",Write Data:%d ",DataBuf.u16Data);
	Write_Flash(FLASH_OP_ADDR+Flash_addr,&DataBuf.u8Data[0],2);	
}
  ...
  /* Infinite loop */
{
	...
	ledGpioInit();
	USART_Configurature();
	printf("Flash Program / Erase by HalfWord test\r\n");
