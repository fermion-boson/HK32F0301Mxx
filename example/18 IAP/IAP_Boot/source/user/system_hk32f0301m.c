...
#define SYSCLK_SRC_HSI12M		0x2
#define SYSCLK_SRC_HSI24M		0x3
...
#define VECT_TAB_OFFSET  0x00 /*!< Vector Table base offset field. This value must be a multiple of 0x200. */
	...
	#define SYSCLK_FREQ_HSI_12M    			12000000 
  	uint32_t SystemCoreClock         = SYSCLK_FREQ_HSI_12M;        					/*!< System Clock Frequency (Core Clock) */
  	static void SetSysClockToHSI_12M(void);		
#elif(SYSCLK_SOURCE == SYSCLK_SRC_HSI24M)
	#define SYSCLK_FREQ_HSI_24M    			24000000
	uint32_t SystemCoreClock         = SYSCLK_FREQ_HSI_24M;        					/*!< System Clock Frequency (Core Clock) */
  	static void SetSysClockToHSI_24M(void);		
#elif(SYSCLK_SOURCE == SYSCLK_SRC_HSI48M)
	#define SYSCLK_FREQ_HSI_48M    			HSI_VALUE
...
static void SetSysClock(void)
{
	...
	SetSysClockToHSI_8M();	
#elif(SYSCLK_SOURCE == SYSCLK_SRC_HSI12M)
	SetSysClockToHSI_12M(); 	
#elif(SYSCLK_SOURCE == SYSCLK_SRC_HSI24M)
	SetSysClockToHSI_24M(); 	
#elif(SYSCLK_SOURCE == SYSCLK_SRC_HSI48M)
...
static void SetSysClockToHSI_8M(void)
{
	...
	RCC->CFGR4 |= (((uint32_t)0x0B) << RCC_RCC_CFGR4_FLITFCLK_PRE_Pos);
	...
	/* HCLK = SYSCLK */
	RCC->CFGR = (uint32_t)(RCC_CFGR_HPRE_DIV6|RCCHCLKReg);
